...
main :: IO ()
main = defaultMain
    [ bench "bytestring-read"   $ nf (R.signed R.floating :: ByteString -> Maybe (Double, ByteString)) testB
    , bench "text"   $ nf (T.signed T.double . T.decodeUtf8) testB
    , bench "bytestring-lexing" $ nf L.readDouble testB
    , bench "attoparsec" $ nf (\s -> A.parseOnly (fmap toRealFloat A.scientific :: A.Parser Double) s) testB
    , bench "read" $ nf (\a -> reads $ SC.unpack a :: [(Double, String)]) testB
    ]
